...
},
{
    ...
    "language": "en",
    "country": "us",
    "published_at": "2020-09-25T23:23:14+00:00"
}];
...
articles.sort(function(a, b) {
        let x = new Date(a.published_at);
        let y = new Date(b.published_at);
        return ((x.getTime() < y.getTime()) ? -1 : ((x.getTime() > y.getTime()) ? 1 : 0));
    });

articles.sort((a,b) => new Date(a.published_at).getTime() - new Date(b.published_at).getTime()); //ES6

// Sort array of articles by author name (A-Z).
...
// Write a function that takes one parameter - author name and returns his last article (last means the most latest by date)
...
findLastArticle ('Michael Rothstein');